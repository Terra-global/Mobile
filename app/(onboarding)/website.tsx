...
export default function WebsiteScreen() {
  ...
  const [value, setValue] = React.useState(user?.website || 'https://');
  const [loading, setLoading] = React.useState(false);

  const handleUpdate = async () => {
    setLoading(true);
    try {
      await api.patch('/users/profile', { website: value });
      updateUser({ website: value });
    ...
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <View style={[styles.container, { backgroundColor: themeColors.background, paddingTop: Math.max(insets.top, 20) }]}>
        ...
        <View style={styles.topBar}>
          ...
          <Text style={[styles.topBarTitle, { color: themeColors.subtext }]}>Website</Text>
